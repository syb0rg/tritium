...
static cst_val *us_tokentowords_one(cst_item *token, const char *name)
{
    ...
    if ((streq("a",name) || streq("A",name)) &&
        ((item_next(token) == 0) ||
         (!streq(name,item_name(token))) ||
         (!streq("",ffeature_string(token,"punc")))))
	...
	if (streq("",ffeature_string(token,"punc")))
	...
	if (streq("nide",nsw))
	    ...
	    else if (streq(ts,"digits"))
		r = en_exp_digits(name);
	    else if (streq(ts,"year"))
		r = en_exp_id(name);
	    else
    ...
    {   /* Roman numerals */
	if (streq("",ffeature_string(token,"p.punc")))
	{   /* no preceeding punc */
	    char n[10];
	...
	else 
	{
		...
		r = cons_val(string_val(street),NULL);
		     ...
		     (nname[0] >= 'A') && (nname[0] <= 'Z'))
		r = cons_val(string_val(saint),NULL);
	    else if (streq(ffeature_string(token,"n.whitespace")," "))
		r = cons_val(string_val(saint),NULL);
	    else
		r = cons_val(string_val(street),NULL);
	}
	if (streq(item_feat_string(token,"punc"),"."))
	    item_set_string(token,"punc","");
    }
    else if (streq(name,"Mr"))
    {
	item_set_string(token,"punc","");
	r = cons_val(string_val("mister"),NULL);
    }
    else if (streq(name,"Mrs"))
    {
	item_set_string(token,"punc","");
    ...
    else if (streq(name,"am") || streq(name,"AM"))
    {
        if (!streq(name,item_name(token)))
            r = en_exp_letters(name);
	     ...
	     (name[0] >= 'A') &&
	     (name[0] <= 'Z') &&
	     (streq(" ",ffeature_string(token,"n.whitespace"))) &&
...
/*	printf("money, money, money %s\n", name); */
	...
	else if (streq(p,"'tve")) /* admittedly rare and weird */
	{
	    aaa = cst_strdup(name);
    ...
    {   /* weights and measures */
        ...
        for (i=0; wandm_abbrevs[i][0]; i++)
            if (streq(name+j,wandm_abbrevs[i][0]))
                break;
        aaa = cst_strdup(name);
        aaa[j] = '\0';
        /* remove any commas */
...
static cst_val *state_name(const char *name,cst_item *t)
{
    ...
    {
	if (streq(states[s][0],name))
	{
	    if (streq(states[s][1],"ambiguous"))
	    {

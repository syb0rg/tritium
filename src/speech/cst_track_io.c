...
int cst_track_load_est(cst_track *t, const char *filename)
{
    ...
    while (!streq("EST_Header_End",(tok=ts_get(ts))))
    {
	if (streq("DataType",tok))
	{
	    tok = ts_get(ts);
	    if (streq("ascii",tok))
	    {
	...
	else if (streq("ByteOrder",tok))
	{
	    tok = ts_get(ts);
	    swap = (streq(tok, BYTE_ORDER_BIG) && CST_LITTLE_ENDIAN)
		|| (streq(tok, BYTE_ORDER_LITTLE) && CST_BIG_ENDIAN);
	}
	else if (streq("NumFrames",tok))
	    num_frames = atoi(ts_get(ts));
	else if (streq("NumChannels",tok))

...
const cst_val *cg_state_pos(const cst_item *p)
{
    ...
    if (!streq(name,ffeature_string(p,"p.name")))
        return (cst_val *)&val_string_pos_b;
    if (streq(name,ffeature_string(p,"n.name")))
...
{
    if (p && streq("pau",item_feat_string(p,"name")))
        return &val_int_1;
    else
        return &val_int_0;
}
...
static const cst_val *seg_coda_ctype(const cst_item *seg, const char *ctype)
{
    ...
    for (s=item_last_daughter(item_parent(item_as(seg,"SylStructure")));
	 ...
	 s=item_prev(s))
    {
	if (streq("+",phone_feature_string(ps,item_feat_string(s,"name"),
					       "vc")))
	    return VAL_STRING_0;
	if (streq(ctype,phone_feature_string(ps,item_feat_string(s,"name"),
					       "ctype")))
	    return VAL_STRING_1;
    }
...
static const cst_val *ssyl_in(const cst_item *syl)
{
    ...
    for (c=0, p=item_prev(ss); 
	 ...
	 p=item_prev(p))
    {
	if (streq("1",item_feat_string(p,"stress")))
	    c++;
    }
...
{
    /* phrase prediction is so different between tritium and festival */

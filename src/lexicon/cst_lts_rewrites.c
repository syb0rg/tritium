		      ...
		      const cst_val *sets)
{
    ...
    if (streq(val_string(PATT),val_string(THING)))
	return TRUE;

    for (sss=sets; sss; sss=val_cdr(sss))
    {
	if (streq(val_string(val_car(val_car(sss))),val_string(PATT)))
...
/*    printf("STRING: "); val_print(stdout, STRING); printf("\n"); */
	...
	r = FALSE;
	     ...
	     (streq("*",val_string(val_car(PATTERN)))))
    {
			...
			const cst_val *sets)
{
    ...
    {
	if (!rc) return FALSE;
	if (!streq(val_string(val_car(ra)),

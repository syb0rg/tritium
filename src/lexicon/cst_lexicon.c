...
{   /* if replace then replace entry in addenda of lex with entry */
    ...
    word = cst_strdup(w);
    p = ts_get(e);
    if (!streq(":",p)) /* there is a real pos */
    {
        pos = cst_strdup(p);
        p = ts_get(e);
        if (!streq(":",p)) /* there is a real pos */
    ...
    while (!ts_eof(e))
    {
        ...
        for (i=0; lex->phone_table[i]; i++)
            if (streq(p,lex->phone_table[i]))
                break;
        if (streq("#",p)) /* comment to end of line */
            break;
...
void lexicon_register(cst_lexicon *lex)
{
    ...
    old_lexs = tritium_lexicons;
    tritium_num_lexicons++;
    tritium_lexicons = cst_alloc(cst_lexicon *,tritium_num_lexicons);
    for (i=0; i<tritium_num_lexicons-1; i++)
	tritium_lexicons[i] = old_lexs[i];
    tritium_lexicons[i] = lex;
    cst_free(old_lexs);
}
...
cst_lexicon *lexicon_select(const char *name)
{
    ...
    for (i=0; i < tritium_num_lexicons; i++)
	if (streq(name,tritium_lexicons[i]->name))
	    return tritium_lexicons[i];
    return NULL;
}
#endif
...

{
	...
	if (!streq(word+1,word_pos+1))
	    break;
	else if (streq(word,word_pos))
	    return w;
